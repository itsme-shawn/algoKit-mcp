...
algokit는 **데이터 기반 학습 분석**과 **AI 기반 개인화 피드백**을 통해 효율적인 알고리즘 학습을 지원합니다.
...
#### 2.2 학습 지원
- **get_hint**: 단계별 힌트 생성 (3단계: 문제 분석 → 핵심 아이디어 → 상세 접근법)
...
```bash
# 새 브랜치로 worktree 생성
git worktree add ../algokit-feature-search feature/search-problems

# 기존 브랜치로 worktree 생성
git worktree add ../algokit-bugfix bugfix/api-error

# 특정 커밋에서 새 브랜치로 worktree 생성
git worktree add -b hotfix/critical ../algokit-hotfix abc1234
```
...
```bash
# 메인 개발 디렉토리
/Users/shawn/dev/projects/AlgoKit (main)

# 새 기능 개발용 worktree
git worktree add ../algokit-feat-analytics feature/user-analytics

# 작업 후 커밋
cd ../algokit-feat-analytics
# ... 개발 및 커밋 ...
...
git worktree add ../algokit-hotfix -b hotfix/api-timeout main
cd ../algokit-hotfix
# ... 버그 수정 및 커밋 ...
...
git worktree add ../algokit-review-pr123 origin/feature/hint-generator

# 리뷰 및 테스트
cd ../algokit-review-pr123
npm test
# ... 코드 검토 ...

# 리뷰 완료 후 제거
cd ../AlgoKit
git worktree remove ../algokit-review-pr123
```
...
3. **디렉토리 명명 규칙**
   ...
   - 예: `algokit-feat-search`, `algokit-fix-timeout`
